...
def get_labels_attr(dataset):
    label_list_set = {
        ...
        #'iemocap':["happy", "sad", "neutral", "angry", "excited", "frustrated", "disappointed", "fear", "annoyed", "surprise", "depressed", "contempt", "amused", "concerned", "confused", "disgust"],
        ...
        'new':['happy','sad', 'neutral', 'angry', 'excited', 'frustrated','depressed','disappointed','guilt'],
        #'msp': ["Anger", "Frustration", "Disgust", "Annoyance", "Sadness","Depression", "Disappointment", "Fear", "Happiness", "Surprise","Excitement", "Contempt", "Amusement", "Concern", "Confusion", "Neutral"]
        ...
        #'msp':['Anger','Happiness','Neutral','Disgust','Fear','Sadness']
        #'msp': ["angry", "disgust", "sad", "fear", "happy", "neutral"]
        #'msp': ['happy', 'sad', 'neutral', 'angry', 'excited', 'frustrated']
    }
    label_str_set = {
        'iemocap':"'happy', 'sad', 'neutral', 'angry', 'excited', 'frustrated'",
        #'iemocap': "'happy', 'sad', 'neutral', 'angry', 'excited', 'frustrated', 'disappointed', 'fear', 'annoyed', 'surprise', 'depressed', 'contempt', 'amused', 'concerned', 'confused', 'disgust'",
...
def report_score(dataset, golds, preds, mode='test'):
    if dataset == 'iemocap':
        ...
        digits = 6
    #if dataset == 'iemocap':
    #    target_names = ["happy", "sad", "neutral", "angry", "excited", "frustrated", "disappointed", "fear", "annoyed", "surprise", "depressed", "contempt", "amused", "concerned", "confused", "disgust"]
    #    digits = 16
        ...
        target_names = ['happy', 'sad', 'neutral', 'angry','excited', 'frustrated','depressed','disappointed','guilt']
        digits = 9
        
    elif dataset == 'msp':
        #target_names = ['Anger', 'Frustration', 'Disgust', 'Annoyance', 'Sadness', 'Depression', 'Disappointment', 'Fear', 'Happiness', 'Surprise', 'Excitement', 'Contempt', 'Amusement', 'Concern', 'Confusion', 'Neutral']
        #digits = 16
        #target_names = ['Anger','Happiness','Neutral','Disgust','Fear','Sadness']
        target_names = ["angry", "frustrated", "disgust", "annoyed", "sad","depressed", "disappointed", "fear", "happy", "surprise","excited", "contempt", "amused", "concerned", "confused", "neutral"]
        #target_names = ["angry", "disgust", "sad", "fear", "happy", "neutral"]
    ...
    res_matrix = metrics.classification_report(golds, preds, target_names=target_names, labels=all_labels, digits=digits)
...
parser.add_argument(
    ...
    type=str,
    required=True,
    choices=['iemocap','meld','EmoryNLP','new','msp'],
...
def _get_input_dict(batch):
    input_ids, labels, attention_mask = batch["input_ids"], \
...
class DynamicPromptCollator:
    def __init__(self, tokenizer, group_order, max_len=2048, mode='dev'):
        self.tokenizer = tokenizer
        self.group_order = group_order  # Now accepts the nested dictionary
        self.max_len = max_len
        self.mode = mode
        self.epoch = 0
        self.max_mask_prob = 0.2 
            ...
            for key, name in vad_keys.items():
                if key in item:
                   # Get the value (e.g. "5.5/7")
                    ...
                    desc_parts.append(f"{name}: Unknown")
...
if args.do_train:
    ...
    if args.checkpoint_dir is not None:
        ...
        print("checkpoint_dir is provided: printing client state")
        #print(client_state)
    else:
                    ...
                    if "chatglm" in args.model_name_or_path:
                        ...
                        outputs = model.generate(
                            **eval_batch,
                            ...
                            top_p=args.top_p,
                            early_stopping=True,
                            max_new_tokens=8,  # instead of max_length
                            temperature=0.0,   # deterministic decoding for classification
                            repetition_penalty=1.0,
                            do_sample=False,
                        )
