...
def process_csv(input_file, output_file):
    ...
    # df = df[['id', 'emotion', 'path', 'gender','text','valence','arousal','dominance']]
...
def process_new_dataset(input_file, output_file):
    # Read the input CSV file
    df = pd.read_csv(input_file)
    ...
    # Combine the selected columns from the original dataframe with the new features
    result_df = pd.concat([df, features_df.drop('filename', axis=1)], axis=1)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["meld", "iemocap","msp"], help="Dataset to process")
    ...
    if args.dataset == "meld":
        ...
        input_file = 'data/MSP_dataset/podcast_dataset_cleaned_2split.csv'
        output_file = 'speech_features/msp_podcast_features.csv'
        process_new_dataset(input_file, output_file)
