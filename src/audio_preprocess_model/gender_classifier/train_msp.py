...
df = pd.read_csv('/home/FYP/jyau005/SpeechCueLLM-main/speech_features/egemaps_features_filtered.csv')
df = df[['id','gender','split','audio_filepath']]
...
def preprocess_function(examples):
    ...
    for path in examples["audio_filepath"]: 
...
training_args = TrainingArguments(
    output_dir="./wav2vec2-gender-classifier_msp",
    eval_strategy="epoch",
    save_strategy="epoch",
    logging_steps=10,
    ...
    max_grad_norm=1.0,       # FORCES the gradient spikes to be chopped off at 1.0. No more explosions.
    warmup_ratio=0.1,        # Spends the first 10% of training slowly ramping up the learning rate from 0 to 3e-5
    weight_decay=0.01,
)
...
trainer.train()
trainer.save_model("./wav2vec2-gender-best-model_msp")
